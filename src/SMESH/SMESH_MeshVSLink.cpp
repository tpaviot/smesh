...
// Purpose  : compute the interpolated normal at the vertex
// requires for smooth shading
//================================================================
Standard_Boolean SMESH_MeshVSLink::GetNodeNormal(const Standard_Integer ranknode, const Standard_Integer Id,
                                                 Standard_Real &nx, Standard_Real &ny, Standard_Real &nz) const {
...
//										   mesh is a SMESH_Mesh
//			const SMDS_MeshElement* elem = mesh->GetMeshDS()->FindElement(id_type.first);
    const SMDS_MeshElement *elem = this->myMesh->GetMeshDS()->FindElement(Id);


//			if(!elem) RETURN_BAD_RESULT("GetNodeNormal: elem is NULL");
    if (!elem) return Standard_False;

    const SMDS_MeshNode *node = elem->GetNode(ranknode - 1);
...
//			XYZ_ N=NodalNormals[node->GetID()];
