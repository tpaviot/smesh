...
SMESH_MeshVSLink::SMESH_MeshVSLink(const SMESH_Mesh *aMesh, SMESH_subMesh *aSubMesh, int displaymode)
...
//  std::set<SMESHDS_GroupBase*>& groups = mesh->GetMeshDS()->GetGroups();
  if (!groups.empty()) {
...
void SMESH_MeshVSLink::SetDisplayMode(int displaymode)
{
  _displaymode = displaymode; // 1:MeshVS_DMF_WireFrame, 2:MeshVS_DMF_Shading, 3:MeshVS_DMF_Shrink
...
void SMESH_MeshVSLink::GetHiddenElements(int planedir, double planevalue, TColStd_PackedMapOfInteger& hiddenelements)
{
...
Standard_Boolean SMESH_MeshVSLink::GetGeom(const Standard_Integer ID, const Standard_Boolean IsElement,
...
Standard_Boolean  SMESH_MeshVSLink::Get3DGeom(const Standard_Integer ID, Standard_Integer& NbNodes,
...
Standard_Boolean SMESH_MeshVSLink::GetGeomType(const Standard_Integer ID, const Standard_Boolean IsElement,
						MeshVS_EntityType& Type) const
{
  if( IsElement ) return myElements.GetElementType(ID, Type);
    ...
    {
      const SMDS_MeshNode* myNode = mesh->GetMeshDS()->FindNode(ID);
//      SMDS_MeshNode* myNode = mesh->GetMeshDS()->FindNode(ID);
      if (!myNode) return Standard_False;
      if (myNode->GetType() == SMDSAbs_Node)
	Type = MeshVS_ET_Node;
      else
...
Standard_Boolean SMESH_MeshVSLink::GetNodesByElement( const Standard_Integer ID,TColStd_Array1OfInteger& NodeIDs,Standard_Integer& NbNodes ) const
{
...
Standard_Address SMESH_MeshVSLink::GetAddr( const Standard_Integer, const Standard_Boolean ) const
{
  return NULL;
}
