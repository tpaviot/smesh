...
TEST(MeshBasicGeometriesSuite, testMeshBoxNetgen)
{
    // create a box, mixing integers and floats
    BRepPrimAPI_MakeBox my_box(10.,10.,10.);
    my_box.Build();
    ...
    ASSERT_LT(diagonal_size, 17.320509);
    // create and add hypothesis
    NETGENPlugin_Hypothesis_2D* hyp2d = new NETGENPlugin_Hypothesis_2D(0,0,meshgen);
    hyp2d->SetFineness(NETGENPlugin_Hypothesis_2D::Fineness(3.0));
    NETGENPlugin_NETGEN_2D* alg2d = new NETGENPlugin_NETGEN_2D(1,0,meshgen);
    mesh->AddHypothesis(mesh->GetShapeToMesh(), 0);
    mesh->AddHypothesis(mesh->GetShapeToMesh(), 1);
    // compute the mesh
    meshgen->Compute(*mesh, mesh->GetShapeToMesh());
